...
import {bracketMatching, syntaxHighlighting, indentOnInput, foldKeymap, HighlightStyle, LRLanguage} from "@codemirror/language";
 ...
 */
const TAB_KEY_MAP = {key: "Tab", run: commands.insertTab, shift: commands.indentLess};
...
export const languageJavaScript = memo(() => ({
    extensions: [
        javascript(),
        highlighting(JS_HIGHLIGHT_STYLE_SPECS)
    ]
}));
...
export const languageCSS = memo(() => ({
    extensions: [
        css(),
        highlighting(CSS_HIGHLIGHT_STYLE_SPECS)
    ]
}));
...
export const languageHTML = memo(() => {
    ...
    return ({
        extensions: [
            ...
            scopedHighlighting(JS_HIGHLIGHT_STYLE_SPECS, javascriptLanguage, 'Script'),
 ...
 */
export function initCodeEditor(editorContainer, initialContent, language) {
