...
function build(pages) {
    for (let page in pages) {
        ...
        if (type.indexOf('tabbed') > -1) {
            ...
            try {
                console.info(`MDN-BOB: Writing ${currentPage.fileName}....`);
                fse.outputFileSync(outputPath, outputHTML);
            } catch (error) {
                console.error(
                    `MDN-BOB: (pageBuilder.js/@build) Error while building ${
    ...
    return new Promise((resolve, reject) => {
        ...
        for (const metaJson of metaJSONArray) {
            ...
            try {
                ...
                console.error(
                    `MDN-BOB: (pageBuilder.js/@buildPages) Error while building pages: ${metaJson}`
                );
                reject(
                    Error(
                        `MDN-BOB: (pageBuilder.js/@buildPages) Error while building pages: ${metaJson}`
